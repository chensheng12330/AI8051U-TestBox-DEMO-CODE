...
//	#define  CHANNEL  2			  //设置ADC通道(咪头通道),   取值为0~7, 对应P1.0~P1.7, 使用别的ADC输入口则要修改ADC初始化函数.
	#define  CHANNEL  3			  //设置ADC通道(示波器通道), 取值为0~7, 对应P1.0~P1.7, 使用别的ADC输入口则要修改ADC初始化函数.
...
sbit	P_V_C    = P0^2;	//输入增益选择, 倍数   2   4   10  1   20 200  40 100
...
u16	adc_wr, adc_rd;
u8	adc;
u8	SampleIndex;	//0~20对应时基10us~50S, 0:10us 1:20us 2:50us 3:100us 4:200us 5:500us 6:1ms 7:2ms 8:5ms 9:10ms 10:20ms 11:50ms 12:100ms 13:200ms 14:500ms 15:1s 16:2s 17:5s 18:10s 19:20s 20:50s
u16	sample_cnt;
bit	B_sample_ms;
u16	SampleTime;
bit	B_Run;
...
#define	D_ADC_SPEED	0		/* 0~15, ADC时钟 = SYSclk/2/(n+1) */
#define	D_RES_FMT	(0<<5)	/* ADC结果格式 0: 左对齐, ADC_RES: D9 D8 D7 D6 D5 D4 D3 D2, ADC_RESL: D1 D0 0  0  0  0  0  0 */
...
void CMP_ISR(void) interrupt CMP_VECTOR		//比较器中断函数, 检测到反电动势过0事件
{
	ADC_RES = 0;
//	ADC_RESL = 0;
	ADC_CONTR = D_ADC_POWER | D_ADC_START | CHANNEL;	//ADC on + channel, 启动ADC转换+清除完成标志
	TR0 = 1;		//开始运行Timer1.
...
void PWMA_config(void)
{
	...
	ps    |= 0;				// 选择IO, 0:选择P1.0 P1.1, 1:选择P0.0 P0.1, 2:选择P2.0 P2.1,
...
void PWMB_config(void)
{
	...
	PWMB_CCR2H  = 0;		// 比较值, 控制占空比(高电平时钟数)
	PWMB_CCR2L  = 87;
	ccer1 |= 0x50;			// 开启比较输出, 高电平有效
	ps    |= (2<<2);		// 选择IO, 0:选择P0.3, 1:选择P1.3, 2:选择P2.3, 3:选择P5.1
	eno   |= 0x04;			// IO输出允许,  bit6: ENO8P, bit4: ENO7P,  bit2: ENO6P,  bit0: ENO5P
	P2n_push_pull(Pin3);
//	PWMB_ISR_En|= 0x04;		// 使能中断
...
//    00       P0.7    P0.5    P0.3    P0.1
//    01       P1.7    P1.5    P1.3    P1.1
//    02       P2.7    P2.5    P2.3    P2.1
//    03       P5.3    P5.2    P5.1    P5.0
...
//	sr1 &= PWMB_ISR_En;	//每个通道可以单独允许中断处理
	if(sr1 & 0x01)	//更新中断标志
	{
		PWMB_CCR2L = T_SIN[SineIndex];		//P2.3输出1000Hz正弦波
...
void	SetSampleTime(void)
{
	u16	j;
		 ...
		 if(SampleIndex <= 5)	ADCCFG = D_RES_FMT + 0;	// ADC CLOCK = 20MHz @2us/sample,  ADC CLOCK = SYSclk/2/(n+1), 转换需要 28 ADC CLOCK
	else if(SampleIndex == 6)	ADCCFG = D_RES_FMT + 1;	// ADC CLOCK = 10MHz @4us/sample,  ADC CLOCK = SYSclk/2/(n+1)
	else if(SampleIndex == 7)	ADCCFG = D_RES_FMT + 3;	// ADC CLOCK =  5MHz @8us/sample,  ADC CLOCK = SYSclk/2/(n+1)
	else 						ADCCFG = D_RES_FMT + 9;	// ADC CLOCK =  2MHz >=20us/sample, ADC CLOCK = SYSclk/2/(n+1)
