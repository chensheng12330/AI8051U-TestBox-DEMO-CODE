...
通过USB CDC(Communication Device Class)协议识别为串口设备连接ISP软件(V6.90以后版本);

ISP软件调试接口里面的虚拟键盘上点击按键发送相应的键值到MCU;
...
2. 通过加载“usb_cdc_32.lib”库函数，实现使用ISP软件发送指令触发MCU复位，进入USB下载模式并自动下载。
   (注意：使用CDC接口触发MCU复位并自动下载功能，需要勾选端口设置：下次强制使用”USB Writer (HID)”进行ISP下载)
